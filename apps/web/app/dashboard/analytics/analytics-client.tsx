        ...
        <div className="max-h-[600px] overflow-y-auto">
          {links.map((link) => (
            <button
              ...
              className={`flex w-full items-center justify-between px-4 py-3 text-left text-sm transition-colors hover:bg-muted/50 border-b border-border last:border-b-0 ${
                selectedLink === link.id ? 'bg-primary/5' : ''
              }`}
            >
              ...
              <div className="flex items-center gap-3 shrink-0 ml-4">
                ...
                <ChevronDown
                  size={14}
                  className={`transition-transform ${selectedLink === link.id ? 'rotate-180' : ''}`}
                />
              </div>
            </button>
          ))}
