...
async function main() {
  ...
  const upsertTier = async (
    ...
    prisma.tier.upsert({
      ...
      update: data as any,
      create: { id, code: code as any, ...data } as any,
    });

  const tiers = [];
  tiers.push(
    await upsertTier("FREE", "tier_free", "Free", {
      name: "Free",
      maxLinks: 5,
      maxLinksPerMonth: 5,
  ...
  tiers.push(
    await upsertTier("PRO", "tier_pro", "Pro", {
      name: "Pro",
      maxLinks: null,
      ...
      priceAnnual: 12000,
      description:
        "For professionals who need unlimited links and advanced features",
      features: [
        "Unlimited links",
        "Expiration controls",
        "Advanced analytics",
  ...
  tiers.push(
    await upsertTier("DEVELOPER", "tier_developer", "Developer", {
      name: "Developer",
      maxLinks: null,
      maxLinksPerMonth: null,
      maxQrCodesPerMonth: null,
      ...
      priceAnnual: 27600,
      description:
        "For developers who want API-first workflow and team features",
      features: [
        "Unlimited links",
        "API key access",
  ...
  console.log(
    `Seeded ${tiers.length} tiers: ${tiers.map((t) => t.code).join(", ")}`,
  );
